...
Conteo físico realizado por **dos personas** conectadas desde celulares. Cada uno registra **líneas independientes** con desglose (pallet × unidades, sueltos). Al finalizar ambos un sector, comparan totales; recontean solo productos con diferencia hasta dar cero. Ver documentos:

- [INVENTARIO.md](INVENTARIO.md) — flujo completo
- [DESGLOSE-DE-CANTIDADES.md](DESGLOSE-DE-CANTIDADES.md) — formato de cantidades

**Resumen:**
1. Sesión con sectores y dos contadores por sector.
2. Buscador dinámico o escaneo para seleccionar producto.
3. Líneas de conteo independientes por ubicación (no se fusionan en pantalla).
4. Al finalizar ambos → comparación visible para los dos; reconteo solo donde hubo diferencia.
5. Todos los sectores OK → comparación total vs stock del sistema.
...
| R5 | Inventario: dos contadores distintos por sector; vistas independientes durante conteo; comparación al finalizar ambos; reconteo solo con diferencia |
...
- [ ] Detalle completo del flujo de inventario (ver [INVENTARIO.md](INVENTARIO.md)) — *mayormente definido*
- [ ] ¿Ajustes de stock post-inventario automáticos o solo con aprobación de supervisor?
