...
6. **Vuelta a oficina** — Se conectan otra vez al PC e **importan** el conteo.
...
- El sync principal **no** es WhatsApp/archivo: es **HTTP local por hotspot**. Archivo JSON = **solo respaldo**.
...
| Respaldo por archivo JSON | UI secundaria “Respaldo: enviar archivo JSON” |
...
- Mensajes / UX de “quién es host” y errores de IP.
- Estados de sector en PC tipo `PENDIENTE_IMPORT` (nombre exacto aún informal).
...
5. Con OK: **Importar resultado al PC** (cuando vuelvan a la red de oficina).
...
```
server/utils/inventario-offline.ts     # buildPaqueteOffline, importarConteoOffline
server/routes/inventario.ts            # rutas paquete / import / modo
...
| Archivo JSON | Solo plan B / respaldo |
