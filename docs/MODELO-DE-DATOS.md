...
## Inventario (ver INVENTARIO.md para flujo completo)
...
|-------|------|-------|
| id | PK | |
| nombre | string | ej. "Inventario Junio 2025" |
...
**Comparación:** por producto, suma de `total_unidades` de todas las líneas de cada contador en la ronda actual.

### `inventario_diferencias`

Diferencias detectadas entre contadores o vs sistema.

| Campo | Tipo | Notas |
|-------|------|-------|
| id | PK | |
| sesion_id | FK | |
| inventario_sector_id | FK | Nullable (si es diff por sector) |
| producto_id | FK | |
| tipo | enum | `ENTRE_CONTADORES`, `VS_SISTEMA` |
| cantidad_contador_1 | decimal | |
| cantidad_contador_2 | decimal | |
| cantidad_sistema | decimal | Solo para VS_SISTEMA |
| resuelta | boolean | |
| created_at | datetime | |
