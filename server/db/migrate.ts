...
export function runMigrations(db: Database.Database): void {
  ...
  // Inventario offline: modo por sector + timestamps de paquete/import
  if (tableExists(db, 'inventario_sectores')) {
    if (!columnExists(db, 'inventario_sectores', 'modo_conectividad')) {
      db.exec(`
        ALTER TABLE inventario_sectores
        ADD COLUMN modo_conectividad TEXT NOT NULL DEFAULT 'ONLINE'
      `)
    }
    if (!columnExists(db, 'inventario_sectores', 'paquete_descargado_at')) {
      db.exec(`ALTER TABLE inventario_sectores ADD COLUMN paquete_descargado_at TEXT`)
    }
