...
CREATE TABLE IF NOT EXISTS movimientos_internos (
  ...
  sector_destino_id INTEGER REFERENCES sectores(id),
  observacion TEXT,
  estado TEXT NOT NULL DEFAULT 'PENDIENTE' CHECK (estado IN ('ABIERTA', 'PENDIENTE', 'COMPLETADO', 'CANCELADO')),
...
CREATE TABLE IF NOT EXISTS inventario_sectores (
  ...
  modo_conectividad TEXT NOT NULL DEFAULT 'ONLINE' CHECK (modo_conectividad IN ('ONLINE', 'OFFLINE')),
  modo_verificacion TEXT NOT NULL DEFAULT 'DOBLE' CHECK (modo_verificacion IN ('DOBLE', 'SIMPLE')),
...
-- Agenda de turnos (insumos → Mendoza). Independiente del stock/productos.
CREATE TABLE IF NOT EXISTS insumos_transportistas (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  nombre TEXT NOT NULL,
  activo INTEGER NOT NULL DEFAULT 1,
  created_at TEXT NOT NULL DEFAULT (datetime('now'))
