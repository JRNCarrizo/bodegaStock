...
/** Total en cajas para inventario (misma regla que movimientos/stock). */
...
export function formatTotalesInventarioResumen(
  ...
  const parts: string[] = []
  if (t.cajas > 0) {
    parts.push(`${t.cajas} caja${t.cajas === 1 ? '' : 's'}`)
  }
  if (t.suelto > 0) {
    const u = normalizarUnidadProducto(unidadProducto)
    parts.push(`${t.suelto} ${u}${t.suelto === 1 ? '' : 's'}`)
  }
...
export function applyIngresoLineToStock(
  ...
  if (totalStock <= 0) throw new Error('Línea con total inválido')
  if (params.linea.tipo_bulto !== 'SUELTO' && totalCajas <= 0) {
    throw new Error('Línea con total inválido')
  }
...
function isCajaLlena(linea: StockLineaRow, _botellasPorCaja: number): boolean {
  return linea.tipo_bulto === 'CAJA' && Number(linea.cantidad_bultos ?? 0) > 0
}
...
export function refreshStockSectorTotal(db: Database.Database, stock_sector_id: number): void {
