...
export function ConfiguracionPage() {
  ...
  const loadDesktopPrefs = useCallback(async () => {
    if (!api?.getDesktopPrefs) {
      setDesktopPrefs(null)
      return
    }
    try {
      setDesktopPrefs(await api.getDesktopPrefs())
    } catch {
      setDesktopPrefs(null)
    }
  }, [api])
  ...
  async function updateDesktopPref(partial: Partial<DesktopPrefs>) {
    if (!api?.setDesktopPrefs) return
    setSavingDesktopPrefs(true)
    try {
      const next = await api.setDesktopPrefs(partial)
            ...
            <>
              ...
              {mode === 'server' && (
                <div className="space-y-4 rounded-xl border border-surface-border bg-slate-50/80 p-4 sm:p-5">
                  ...
                  {desktopPrefs && api?.setDesktopPrefs && (
                    <div className="space-y-3 border-t border-surface-border pt-4">
                      <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">
                        Segundo plano (PC servidor)
                      </p>
                      <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-surface-border bg-white p-3.5 shadow-sm">
                        <input
                          ...
                          checked={desktopPrefs.openAtLogin}
                          disabled={savingDesktopPrefs || !appInfo?.isPackaged}
                          onChange={(e) => void updateDesktopPref({ openAtLogin: e.target.checked })}
                        />
                      ...
                      <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-surface-border bg-white p-3.5 shadow-sm">
                        <input
                          ...
                          onChange={(e) => void updateDesktopPref({ closeToTray: e.target.checked })}
                        />
                        <span className="min-w-0">
                          <span className="text-sm font-semibold text-slate-900">
                            Al cerrar la ventana, seguir en segundo plano
                          </span>
                          <span className="mt-1 block text-sm leading-relaxed text-slate-500">
                            La X oculta a la bandeja y el servidor sigue activo. Para apagar del
                            todo: clic derecho en el ícono → Salir.
                          </span>
                        </span>
                      </label>
                    </div>
