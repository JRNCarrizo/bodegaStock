...
export function InventarioPage() {
  ...
  }, [canManageInventario, refreshInventarioActivo, loadMisSectores])
  ...
  usePolling(
    () => loadMisSectores(),
    view === 'list' && canCount && activo != null
  )
            ...
            <div className="space-y-2">
              {misSectores.map((sec) => (
                <button
                  key={sec.id}
                  type="button"
                  onClick={() => navigate(`/inventario/contar/${sec.id}`)}
                ...
                >
                  <div>
                    <p className="font-medium text-slate-800">{sec.sector_nombre}</p>
                    <p className="text-xs text-slate-500">
                      Con {sec.soy_contador_1 ? sec.contador_2_nombre : sec.contador_1_nombre} · Ronda {sec.ronda_actual}
                    </p>
                  </div>
                  <span className={cn('rounded-full px-2 py-0.5 text-xs font-medium', ESTADO_SECTOR_COLOR[sec.estado])}>
                    {ESTADO_SECTOR_LABEL[sec.estado]}
...
function CrearSesionForm({
  ...
  const [asignaciones, setAsignaciones] = useState<Record<number, { c1: number; c2: number }>>({})
                  ...
                  <div className="mt-2 grid gap-2 sm:grid-cols-2">
                    <select
                      ...
                      onChange={(e) =>
                        setAsignaciones((prev) => ({
                          ...prev,
                          [sec.id]: { c1: Number(e.target.value), c2: prev[sec.id]?.c2 ?? 0 }
                        }))
                      }
                    >
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setAsignaciones((prev) => ({
                          ...prev,
                          [sec.id]: { c1: prev[sec.id]?.c1 ?? 0, c2: Number(e.target.value) }
                        }))
                      }
                    >
            ...
            {selectedProduct && (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-6">
                  <div>
                    ...
                    >
                      <option value="PALLET">Pallet</option>
                      <option value="CAJA">Caja</option>
                      <option value="SUELTO">Suelto / pucherio</option>
                    </select>
                  </div>
                ...
                <p className="mt-3 text-xs leading-relaxed text-slate-500">
                  ...
                  <span className="font-medium text-slate-600">Suelto:</span> solo pucherio, sin bulto.
                </p>
              </div>
            )}
          </div>
